...
class Command(BaseCommand):
    ...
    def sync(self, alias):
        ...
        strategy_class = replication_opts.pop("strategy_class", "SimpleStrategy")
        replication_factor = replication_opts.pop("replication_factor", 1)

        self.stdout.write(
            "Creating keyspace {} [CONNECTION {}] ..".format(keyspace, alias)
        )

        if strategy_class == "SimpleStrategy":
            management.create_keyspace_simple(
                keyspace, replication_factor, connections=[alias]
            )
        else:
            management.create_keyspace_network_topology(
                keyspace, replication_opts, connections=[alias]
            )
